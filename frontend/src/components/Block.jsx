...
function Block({ block, index, updateBlock, rehashBlock, blocks, showTick, confirmBlock, clientId }) {
  ...
  return (
    <div className={getBlockClass(isSubsequentBlocksInvalid, block)}>
      ...
      <div>
        <p className="text-sm mt-2 break-all"><strong>Previous Hash:</strong> {formatHash(block.previousHash)}</p>
        <p className="text-sm mt-2 break-all"><strong>Current Hash:</strong> {formatHash(block.hash)}</p>
        <p className="text-sm mt-2 break-all"><strong>Block Proposed by:</strong> {formatHash(block.addedBy)}</p>
